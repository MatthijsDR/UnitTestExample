...
namespace UnitTestingExample.WorkUnit.Final.Tests
{
    ...
    public class WorkUnitTests
    {
        ...
        public void InvalidNValueTest()
        {
            ...
            // Assert
            act.Should().Throw<ArgumentException>($"Must be a positive integer{Environment.NewLine}Parameter name: n"); // act should test if ArgumentException is thrown with message $"Must be a positive integer{Environment.NewLine}Parameter name: n"
        }
        ...
        public void TestShortcutForNIs0()
        {
            ...
            // Assert
            result.Should().Be(0); // result should be 0; moqLogger should verify if a string (any string) was logged exactly twice
        ...
        public void TestShortcutForNIs1()
        {
            ...
            // Assert
            result.Should().Be(1);
            moqLogger.Verify(m => m.Log(It.IsAny<string>()), Times.Exactly(2)); // result should be 1; moqLogger should verify if a string (any string) was logged exactly twice
        }
    }
}
